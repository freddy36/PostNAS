...
*/
	...
	// Den URL-Parameter "&id=j/n" in allen Kombinationen umschalten lassen.
	// Die Parameter &gkz= und &gmlid= kommen in allen Modulen einheitlich vor

	// Der Parameter "&eig=j" wird angehaengt wenn gefuellt.  // ALT
	...
	echo "\n\t<td title='Konfiguration'>";		// Umschalter Testmodus I D - A n z e i g e
		...
		//	if ($eig <> "") echo "&amp;eig=".$eig; // nur fuer FS-Nachw
			if ($append <> "") echo $append; // nur fuer FS-Nachw

			if ($idanzeige) { // Umschalten
				echo "&amp;id=n' title='Ohne Verfolgung der ALKIS-Beziehungen'>ID aus";
			} else {
				echo "&amp;id=j' title='Verfolgung der GML-ID in den ALKIS-Beziehungen'>ID ein";
			}
		echo "</a>";
	echo "\n\t</td>";
...
function fs_gebietszug($con, $gemeinde, $kreis, $bezirk) {
	...
	echo "Kreis <span class='key'>(".$kreis.")</span> ".$knam."<br>";
	...
	echo "Regierungsbezirk <span class='key'>(".$bezirk.")</span> ".$bnam."</td>";
...
function fs_lage($con, $gmlid, $gkz) {
...
//	$sql.="AND cast(l.lage AS integer)=cast(s.lage AS integer) "; // Fehlversuch, auch nicht-numerische Inhalte
	...
	return $z; // Summe der ausgegebenen Zeilen
}
...
function bnw_fsdaten($con, $gkz, $idanzeige, $lfdnr, $gml_bs, $ba, $anteil, $bvnraus) {
...
function eigentuemer($con, $gkz, $idanzeige, $gmlid, $mitadresse) {
	...
	while($rown = pg_fetch_array($resn)) {
		echo "\n<tr>";
			...
			if ($rechtsg != "" ) {
				if ($rechtsg == 9999) { // sonstiges
					echo "<p class='zus' title='Beschrieb der Rechtsgemeinschaft'>".htmlentities($rown["beschr"], ENT_QUOTES, "UTF-8")."</p>";
				} else {
					echo "<p class='zus' title='Art der Rechtsgemeinschaft'>".htmlentities(rechtsgemeinschaft($rown["adr"]), ENT_QUOTES, "UTF-8")."</p>";
		...
		while($rowe = pg_fetch_array($rese)) {
			...
			// Spalte 2 = Angaben
			echo "<p class='geig' title='Eigent&uuml;merart ".eigentuemerart($rown["eigentuemerart"])."'>".$diePerson."</p></td>";

			// Spalte 3 = Link			echo "\n\t<td><p class='nwlink noprint'>";
				if ($idanzeige) {linkgml($gkz, $rowe["gml_id"], "Person"); echo "&nbsp";}
				echo "<a href='alkisnamstruk.php?gkz=".$gkz."&amp;gmlid=".$rowe[0];
				if ($idanzeige) {echo "&amp;id=j";}
				echo "' title='vollst&auml;ndiger Name und Adresse eines Eigent&uuml;mers'>Person <img src='ico/Eigentuemer.ico' width='16' height='16' alt=''></a></p>";
			echo "</td>\n</tr>";
